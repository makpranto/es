<table class="table table-striped table-bordered table-hover dataTables-example" >
    ...
    <tbody>
        <?php
        $count = 0;
        $as = r("SELECT * FROM `basket` WHERE `status` = 'quotation_approved'  GROUP BY `price_check_id` ORDER BY `id` ASC");
        while ($a = get($as)) {
            $cid = $a['customer_id'];
            ...
            $ad = r("SELECT * FROM `basket` WHERE `price_check_id` = '$uid' AND `status` = 'quotation_approved'");
            $amount = 0;
            while ($d = get($ad)) {
