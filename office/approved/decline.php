...
if (!there('id') || !there('reason')) {
    ...
    $df = r("SELECT `id` FROM `basket` WHERE `price_check_id` = '$id' AND  `status` = 'pending_int_approval'");
    if (mysqli_num_rows($df) < 1) {
        ...
        if (empty($reason)) {
            ...
            foreach ($ids as $key) {
                r("UPDATE `basket` SET `status` = 'denied_internally', `checked_by` = '$user_id', `checked_at` = '$now', `reason` = '$reason' WHERE `id` = '$key'");
            }
            // echo "<style> #myModal{display: none;}</style>";
            //
