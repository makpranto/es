...
if (!there('id')) {
    ...
    if (empty($uid)) {
        ...
        $as = r("SELECT * FROM `basket` WHERE `uid` = '$uid' AND `status` = 'Order Accepted. Pending Delivery'");
                ...
                <div class="modal-body" style="overflow-y: scroll; height:500px;">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php
                            $count = 0;
                            $ass = r("SELECT * FROM `basket` WHERE `uid` = '$uid' AND `status` = 'Order Accepted. Pending Delivery'");
                            while ($as = get($ass) ) {
                                $count++;
                            ...
                            if ($coupon == '') {
                                ...
                                $coupon .= ' ('.hide('d', pull('coupon', 'coupons', "`id` = '$coupon'")).')';
